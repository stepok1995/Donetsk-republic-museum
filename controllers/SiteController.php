...
if(Yii::$app->request->post())//если есть пост запрос, то находим ИД
  ...
  // ->orderBy(['date'=>SORT_DESC])
	...
	public function actionDayvictory()
	{
			$model=TAction::find()
				->where(['class_id'=>10]);
				$pagination = new Pagination([
		...
		$model_image=TActionImage::find()
		->all();
			  return $this->render('dayvictory',['model'=>$model,'model_image'=>$model_image,'pagination'=>$pagination]);
	}


	public function actionHomemuseum()
	{
			$model=TAction::find()
				->where(['class_id'=>10]);
				$pagination = new Pagination([
		...
		$model_image=TActionImage::find()
		->all();
			  return $this->render('homemuseum',['model'=>$model,'model_image'=>$model_image,'pagination'=>$pagination]);
	}

	public function actionVictoryterritory()
	{
			$model=TAction::find()
				->where(['class_id'=>10]);
				$pagination = new Pagination([
		...
		$model_image=TActionImage::find()
		->all();
			  return $this->render('victoryterritory',['model'=>$model,'model_image'=>$model_image,'pagination'=>$pagination]);
	}
...
$post=0;
  ...
  public function actionAddAdmin() {
    ...
    if (empty($model)) {
        $user = new User();
        $user->username = 'adminMuseumDRKM';
        $user->email = 'admin@yandex.ru';
        $user->setPassword('adminDoneckMuseum24121924');
...
public function actionInteractivemap()
{
          return $this->render('interactivemap');
}
...
public function actionFeedback()
{
	...
	$model_feed = new TFeedback();
	if ($model_feed->load(Yii::$app->request->post()) && $model_feed->save()) 
	{		
	//	return 'yes';				  
			return $this->redirect(['feedback']);
	  }
			return $this->render('feedback',['model'=>$model,'model_feed'=>$model_feed, 'pagination'=>$pagination]);
}
public function actionSiteSaurmogila()
{
          return $this->render('mainsaur');
}
